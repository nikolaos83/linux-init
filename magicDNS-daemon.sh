...
tailscale status --json | jq -r \
  --arg domain "$DOMAIN" '
  .Peer | to_entries[] |
  select(.value.HostName != null) |
  (.value.TailscaleIPs[0] + " " + .value.HostName + "." + $domain)
' > "$TMP_FILE"

grep -v "# TAILSCALE" "$HOSTS_FILE" > "$HOSTS_FILE.tmp"

{
  echo "# TAILSCALE (auto-generated by update-tailscale-hosts.sh)"
  cat "$TMP_FILE"
} >> "$HOSTS_FILE.tmp"
